                ...
                if i == 1:  # zero-indexed: line index 1 == second line
                    ...
                    try:
                        ...
                        if m:
                            ...
                            try:
                                return datetime.strptime(rebuilt, _LOG_DATE_FORMAT).date()
                            except ValueError:
                                pass
                    return None
    except OSError:
...
def check_model_ran(log_dir, parse_log_basename):
    """Return 'Yes' if the corresponding log_1_* file shows output was generated, else 'No'.
...
def build_excel(log_dir, output_file, filter_date=None, filter_has_year=True,
                models_dir=None):
    ...
    for log_file in log_files:
        ...
        sheet = model_type_map.get(model_name, "Fallback Ops") if model_type_map \
            else "Fallback Ops"
...
def main():
    ...
    build_excel(
        log_dir, output_file,
        filter_date=filter_date, filter_has_year=filter_has_year,
