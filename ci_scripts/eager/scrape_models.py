...
def parse_param_count(model_id: str) -> Optional[float]:
...
def parse_tables(lines: list) -> list:
    ...
    while i < len(lines):
        ...
        try:
            arch_idx = next(j for j, c in enumerate(header_cells) if "Architecture" in c)
            model_idx = next(j for j, c in enumerate(header_cells) if c.strip() == "Models")
            hf_idx = next(
                j for j, c in enumerate(header_cells)
                if "HF" in c or "Example" in c
            )
        ...
        while i < len(lines) and "|" in lines[i]:
            ...
            hf_models = [
                m for m in extract_hf_models(cells[hf_idx])
                if "/" in m
            ]
...
def main():
    ...
    parser.add_argument(
        "--type", choices=["llm", "vlm", "both"], default="both",
